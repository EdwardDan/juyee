...
<c:choose>
	...
	<c:otherwise>
		<liferay-ui:journal-article
			articleResourcePrimKey="${assetEntry.classPK}"></liferay-ui:journal-article>
		<div>附件：</div>
			...
			<c:if test="${map['dlFileEntry'].extension=='pdf'}">
				<portlet:renderURL var="zhxxPdfUrl">
					<portlet:param name="mvcPath"
						value="${contextPath}/asset-viewer/display/zhxx-pdf.jsp" />
					<portlet:param name="title" value="${map['dlFileEntry'].title}" />
					<portlet:param name="entryId" value="${entryId}" />
					<portlet:param name="classNameId"
						value="${map['assetEntry'].classNameId}" />
					...
					<portlet:param name="folderId"
						value="${map['dlFileEntry'].folderId}" />
					<portlet:param name="uuid" value="${map['dlFileEntry'].uuid}" />
					<portlet:param name="extension"
						value="${map['dlFileEntry'].extension}" />
				</portlet:renderURL>
				<a href="${zhxxPdfUrl}">${map['dlFileEntry'].title}</a><br>
				<br />
			</c:if>
			<c:if test="${map['dlFileEntry'].extension!='pdf'}">
				<a href="<%=portalUrl%>/documents/${map['assetEntry'].groupId}/${map['dlFileEntry'].folderId}/${map['dlFileEntry'].title}/${map['dlFileEntry'].uuid}">${map['dlFileEntry'].title}</a><br>
			</c:if>
