...
public class CertificateLoginPortlet extends MVCPortlet {
			...
			if (!is) {
				...
				if (!Validator.isNull(UniqueID)) {
					...
					try {
						...
						digitalCertificate.put("version",
								oSE.getCertDetail(1, clientCert));
						digitalCertificate.put("certificateSerialNo",
								oSE.getCertDetail(2, clientCert));
						digitalCertificate.put("signatureAlgorithm",
								oSE.getCertDetail(3, clientCert));
						digitalCertificate.put("issuerCountry",
								oSE.getCertDetail(4, clientCert));
						digitalCertificate.put("issuerOrganization",
								oSE.getCertDetail(5, clientCert));
						digitalCertificate.put("issuerDepartment",
								oSE.getCertDetail(6, clientCert));
						digitalCertificate.put("issuerProvinces",
								oSE.getCertDetail(7, clientCert));
						digitalCertificate.put("issuerName",
								oSE.getCertDetail(8, clientCert));
						digitalCertificate.put("issuerCity",
								oSE.getCertDetail(9, clientCert));
						digitalCertificate.put("issuerEmail",
								oSE.getCertDetail(10, clientCert));
						digitalCertificate.put("startDate",
								oSE.getCertDetail(11, clientCert));
						digitalCertificate.put("expirationDate",
								oSE.getCertDetail(12, clientCert));
						digitalCertificate.put("userCountry",
								oSE.getCertDetail(13, clientCert));
						digitalCertificate.put("userOrganization",
								oSE.getCertDetail(14, clientCert));
						digitalCertificate.put("userDepartment",
								oSE.getCertDetail(15, clientCert));
						digitalCertificate.put("userProvinces",
								oSE.getCertDetail(16, clientCert));
						digitalCertificate.put("userName",
								oSE.getCertDetail(17, clientCert));
						digitalCertificate.put("userCity",
								oSE.getCertDetail(18, clientCert));
						digitalCertificate.put("userEmail",
								oSE.getCertDetail(19, clientCert));
						digitalCertificate.put("key",
								oSE.getCertDetail(20, clientCert));
					} catch (Exception e) {
						// TODO: handle exception
					}
					String digitalCertificateData = digitalCertificate
							.toJSONString();
					User user = null;
					try {
						...
						try {
							createUser(UniqueID, prikeypwd, userName,
									companyId_, resourceRequest,
									digitalCertificateData);// 创建用户
							user = UserLocalServiceUtil.getUserByScreenName(
									companyId_, UniqueID);
	...
	public void createUser(String UniqueID, String prikeypwd, String userName,
			...
			String digitalCertificateData) throws PortalException,
			SystemException {
		...
		try {
			...
			// ExpandoTable eTable =
			// ExpandoTableLocalServiceUtil.getDefaultTable(PortalUtil.getDefaultCompanyId(),
			// User.class.getName());
			// ExpandoColumn eColumn =
			// ExpandoColumnLocalServiceUtil.getColumn(eTable.getTableId(),
			// "digitalCertificate");
			// ExpandoValueLocalServiceUtil.addValue(
			// eTable.getClassNameId(), eTable.getTableId(),
			// eColumn.getColumnId(), user.getUserId(),
