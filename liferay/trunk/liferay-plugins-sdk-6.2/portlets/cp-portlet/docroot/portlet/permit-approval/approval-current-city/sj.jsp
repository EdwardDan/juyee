		...
		<c:forEach items="${applyMaterials }" var="material"
			...
			<tr style="text-align: center" class="fileTr">
				...
				<td style="text-align: center"><input
					name="<portlet:namespace/>sjfs" 
					class="input-mini text-center" value="${material.sjfs}"
	...
	<div class="text-center">
		...
		<div class="btn-group">
			...
			<aui:button-row>
				...
				<c:choose>
					<c:when
						test='<%=message.equals("通过")&&!Validator.isBlank(permit.getSlbh())%>'>
						<liferay-ui:icon
					...
					<c:when
						test='<%=message.equals("不通过")&&!Validator.isBlank(permit.getSlbh())%>'>
						<liferay-ui:icon
							cssClass='<%="workflow-task-" + randomId + " task-change-status-link"%>'
							id='<%=randomId + HtmlUtil.escapeAttribute(transitionName) + "taskChangeStatusLink"%>'
					...
					<c:when
						...
						<liferay-ui:icon
							cssClass='<%="workflow-task-" + randomId + " task-change-status-link"%>'
							id='<%=randomId + HtmlUtil.escapeAttribute(transitionName) + "taskChangeStatusLink"%>'
							image="sjtgx" message="<%=message%>" method="get" />
					</c:when>
					<c:when
						test='<%=message.equals("不通过")&&Validator.isBlank(permit.getSlbh())%>'>
						<liferay-ui:icon
							cssClass='<%="workflow-task-" + randomId + " task-change-status-link"%>'
							id='<%=randomId + HtmlUtil.escapeAttribute(transitionName) + "taskChangeStatusLink"%>'
							image="sjthx" message="<%=message%>" method="get" />
					</c:when>
