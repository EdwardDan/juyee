...
public class OfficeSupplyPortlet extends MVCPortlet {
	...
	public void doView(RenderRequest renderRequest,
		...
		try {
			officeSupplies = OfficeSupplyLocalServiceUtil
					.getOfficeSuppliesByName(keywords, start, end);
		} catch (SystemException e) {
			log.error("getOfficeSuppliesByName(" + keywords + ", " + start
					+ ", " + end + ")");
			e.printStackTrace();
		}
		int officeSuppliesCount = 0;
		try {
			officeSuppliesCount = OfficeSupplyLocalServiceUtil
					.getOfficeSuppliesCountByName(keywords);
		} catch (SystemException e) {
	...
	public void editOfficeSupply(ActionRequest actionRequest,
			ActionResponse actionResponse) throws IOException, PortletException {
		long officeSupplyId = ParamUtil
				.getLong(actionRequest, "officeSupplyId");
		OfficeSupply officeSupply = null;
		if (officeSupplyId != 0) {
			try {
				officeSupply = OfficeSupplyLocalServiceUtil
						.getOfficeSupply(officeSupplyId);
	...
	public void saveOfficeSupply(ActionRequest actionRequest,
			ActionResponse actionResponse) throws IOException, PortletException {
		long officeSupplyId = ParamUtil
				.getLong(actionRequest, "officeSupplyId");
		...
		if (officeSupplyId == 0) {
			...
			officeSupply = OfficeSupplyLocalServiceUtil
					.createOfficeSupply(officeSupplyId);
	...
	public void deleteOfficeSupplies(ActionRequest actionRequest,
			ActionResponse actionResponse) throws IOException, PortletException {
		String deleteOfficeSupplyIds = ParamUtil.getString(actionRequest,
				"officeSupplyIds");
