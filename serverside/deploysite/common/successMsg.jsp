...
<body <c:if test="${msg.isQuickEvent==true}">onload="init()"</c:if>>
<br>
<table width="450" height="300" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
        <td background="${themePath}/images/success.jpg">
            <table width="330" height="230" border="0" align="center" cellpadding="0" cellspacing="0">
                ...
                <c:if test="${(msg.url != null && msg.url != '')||msg.needClose==true}">
                    <tr>
                            ...
                            <c:choose>
                                <c:when test="${startBizFlow == true}">
                                    <input type="button" value="启动流程" onClick="startBizFlow();this.disabled=true;"
                                           class="button_normal_long">
                                    <input type="button" value="跳过" onClick="doReturn();" class="button_normal_long">
                                </c:when>
                                <c:otherwise>
                                    <input type="button" value="<c:choose><c:when test="${msg.needClose==true}">关闭</c:when><c:otherwise>确定</c:otherwise></c:choose>" onClick="doReturn();this.disabled = true;"
