...
<script type="text/javascript">
    ...
    function toExcel() {
        ...
        $("input:checkbox[name=checkbox]:checked").each(function () {
            ...
            actualIndexs += $(this).parents(".tr_light").find(".actualIndex").val() + ",";
        });
        titiles = titiles.substring(0, titiles.length - 1);
        cols = cols.substring(0, cols.length - 1);
        index = index.substring(0, cols.length - 1);
        actualIndexs = actualIndexs.substring(0, actualIndexs.length - 1);
        $("#listGrid").excelExportAdvance("${ctx}/commonPage/exportGridExcute.do?" + encodeURI("cols=" + cols + "&showtitles=" + titiles + "&filename=" + filename + "&index=" + index + "&actualIndexs=" + actualIndexs));
        closeWindow();
    }
</script>
<div class="form_div">
        ...
        <c:forEach items="${bean}" var="item" varStatus="var">
            <tr class="tr_light">
                ...
                <td class="form_content"><input type="checkbox" name="checkbox" value="${item.cols}" checked=checked />
                    ...
                    <input type="hidden" value="${item.actualIndex}" class="actualIndex"/>
                </td>
            </tr>
        </c:forEach>
        <tr class="tr_button">
            ...
            <td class="form_content" style="width:100%">
                <c:if test="${button=='on'}"><input type="button" value="确定" class="button_confirm" onclick="toExcel()" ></c:if>&nbsp;
