...
<script type="text/javascript">
    ...
    function doViewPrjChargedByDept(deptId, deptName) {
        openWindow("查看" + deptName + "所承担的项目", "${ctx}/projRelateDept/gridForPrjRelateDept.do?deptId=" + deptId+"&account=${account}", true, 1061, 531);
    }

    function doViewDeptPersons(deptId, deptName) {
        loadMainPage("${ctx}/projRelatePerson/gridForDeptPersons.do?deptId=" + encodeURI(deptId)+"&account=${account}&deptName=" + encodeURI(deptName) + "&originalUrl=${ctx}/projRelateDept/grid.do?account=${account}&originalLocation=" + encodeURI("项目建设单位列表"), "建设单位 > 人员列表");
    }
