...
<script type="text/javascript">
    ...
    function printView5() {
        window.open("${ctx}/sgPermit/printView5.do?id=${bean.id}", "不予许可通知书", "status=no,toolbar=no,menubar=no,scrollbars=yes,resizable=yes,width=800,hight=600;");
    }
        ...
        <c:if test="${not empty bean.csDate||not empty bean.fhDate||not empty bean.shDate}">
            <fieldset class="form_fieldset" style="width: 96%;">
                ...
                <table cellpadding="0" cellspacing="0" class="form_table">
                    ...
                    <c:if test="${(bean.status==STATUS_SH_PASS)||(bean.status==STATUS_SH_BACK)||(bean.status==STATUS_NB_BACK)}">
                        <c:forEach items="${historyOpinions}" var="item">
                            <c:if test="${item.status==STATUS_CS_PASS && item.projectType.id==bean.projectType.id}">
                                <tr class="tr_dark">
                                    <td class="form_label_right" style="width: 20%;">初审意见：</td>
                        ...
                        <c:forEach items="${historyOpinions}" var="item">
                            <c:if test="${(item.status==STATUS_SH_PASS||item.status==STATUS_SH_BACK||bean.status==STATUS_NB_BACK) && item.projectType.id==bean.projectType.id}">
                                <tr class="tr_dark">
