    ...
    function save(buttonName, status, tab) {
        ...
        if ("" != buttonName) {
            ...
            if (confirm("是否确定执行 " + buttonName + " 操作？")) {
                saveAjaxData("${ctx}/areaSgPermit/save.do", formId);
            }
        } else {
            saveAjaxData("${ctx}/areaSgPermit/save.do", formId, refreshTabParam);
        }
    }
    function refreshTabParam() {
        if ($("#id").val() == "") {
            var ret = getAjaxData("${ctx}/areaSgPermit/getSgPermitId.do?projectTypeId=" + $("#projectType").val());
            if (ret != "") {
