...
<script type="text/javascript">
    $(function () {
        var conf = {
            gridOpts: {
                url: "${ctx}/areaSgPermit/gridDataCustom.do",
                colNames: ['ID',
                    "绿色",
                    "项目性质",
                    '业务编号',
                ...
                gridComplete: function () {  //在此事件中循环为每一行添加修改和删除链接
                    ...
                    for (var i = 0; i < ids.length; i++) {
                        ...
                        if ('' == status || status == '${STATUS_EDIT}' || status == '${STATUS_SLZX_BACK}' || status == '${STATUS_SH_BACK}') {
                            if (${canEdit||isReg}) {
    ...
    function doView(id) {
        openWindow("查看区县施工许可证", "${ctx}/areaSgPermit/frame.do?act=view&id=" + id, false, 850, 500);
    }
    function doAdd() {
        openWindow("选择项目类型", "${ctx}/areaSgPermit/selectProjectType.do", true, 400, 200);
    }
    function doEdit(id) {
        openWindow("修改区县施工许可证", "${ctx}/areaSgPermit/frame.do?act=edit&id=" + id, true, 850, 500);
    }
    function doAccept(id) {
        openWindow("受理区县施工许可证", "${ctx}/areaSgPermit/frame.do?act=accept&id=" + id, true, 850, 500);
    }
    function doAudit(id) {
        openWindow("审核区县施工许可证", "${ctx}/areaSgPermit/frame.do?act=audit&id=" + id, true, 850, 500);
    }
    function doOperation(id) {
        loadMainPage("${ctx}/areaPermitOperation/grid.do?sgPermitId=" + id, "审查备案 > 区县施工许可管理 > 日志");
    }
    function doDelete(id) {
        doGridDelete("${ctx}/areaSgPermit/delete.do?id=" + id);
    }
</script>
