...
<script type="text/javascript">
    $(function () {
        var conf = {
            gridOpts:{
                url:"${ctx}/areaPermitOperation/gridDataCustom.do",
                colNames:['ID',
            	'操作编码',            	          	
            	'操作名称',            	          	
            	'审核操作人',            	          	
...
{name:"status",width:"100",align:"center",searchtype:"integer",sortable:true,formatter:'integer',formatoptions:{thousandsSeparator:",",defaulValue:0}},
...
{name:"updateUser",width:"100",align:"center",searchtype:"string",sortable:true}
    ...
    function doView(id) {
        openWindow("查看区县施工许可流转操作", "${ctx}/areaPermitOperation/view.do?id=" + id, false);
    }
    <c:if test="${canEdit}">
    function doAdd() {
        openWindow("添加区县施工许可流转操作", "${ctx}/areaPermitOperation/add.do", true);
    }
    function doEdit(id) {
        openWindow("修改区县施工许可流转操作", "${ctx}/areaPermitOperation/modify.do?id=" + id, true);
    }    
    function doDelete(id) {
        doGridDelete("${ctx}/areaPermitOperation/delete.do?id=" + id);
    }
    </c:if>
