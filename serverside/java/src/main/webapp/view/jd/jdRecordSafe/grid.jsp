...
<script type="text/javascript">
    $(function () {
        var conf = {
            gridOpts: {
                url: "${ctx}/jdRecordSafe/gridDataCustom.do",
                colNames: ['ID',
                    '选择',
                    '监督记录编号',
                    '报建编号',
                ...
                colModel: [
                    ...
                    {name: "id", width: "10", align: "center", searchtype: "string", formatter: gridRadioFormat},
        ...
        loadJdGridButton('${jdTypeCode}','safe'); //监督记录类别编码
    });
    var TAB_NO = "tab3";
    function doView(id) {
        parent.openWindow("查看过程安全监督记录", "${ctx}/jdRecordSafe/view.do?id=" + id, false, 900, 470);
    }
    <c:if test="${canEdit}">
    function doAdd() {
        parent.openWindow("添加过程安全监督记录", "${ctx}/jdRecordSafe/add.do", true, 900, 470, TAB_NO);
    }
    function doEdit(id) {
        parent.openWindow("修改过程安全监督记录", "${ctx}/jdRecordSafe/modify.do?id=" + id, true, 900, 470, TAB_NO);
    }
    function doDelete(id) {
        parent.doGridDelete("${ctx}/jdRecordSafe/delete.do?id=" + id, null, {iframeId: TAB_NO});
    }
    </c:if>
