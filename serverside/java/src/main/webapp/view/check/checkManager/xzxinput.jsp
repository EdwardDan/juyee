...
<head>
    ...
    <script type="text/javascript">
        ...
        function scanIdCard() {
            ...
            if (str > 0) {
                var nRet;
                SynCardOcx1.SetReadType(0);//手动扫描
                SynCardOcx1.SetPhotoType(2);//图片格式jpeg
                SynCardOcx1.SetPhotoName(2);//图片名为身份证名
                SynCardOcx1.SetPhotoPath(1,"c://");//设置图片保存路径
                nRet = SynCardOcx1.ReadCardMsg();
                if (nRet == 0) {
                    $("#ocxPhoto").show();
                    $("#tPhoto").hide();
                    document.getElementById("name").value = SynCardOcx1.NameA;
                    document.getElementById("cardNo").value = SynCardOcx1.CardNo;
                    document.getElementById("photoFile").value = "/${IDCARD_IMAGE_PATH}/" + SynCardOcx1.CardNo + ".jpg";
//                    document.getElementById("image").innerHTML = "<img src='" + SynCardOcx1.PhotoName + "'/>";
                    document.getElementById("image").value = SynCardOcx1.Base64Photo;
        ...
        function changeDevice(id) {
            if (null == id) {
                openWindow("添加管理人员表", "${ctx}/checkManager/add.do?gdCode=${gdCode}&jdTaskCode=${jdTaskCode}&sWitch=pt", true);
            } else {
                openWindow("修改管理人员表", "${ctx}/checkManager/modify.do?id=${id}&gdCode=${gdCode}&jdTaskCode=${jdTaskCode}&sWitch=pt", true);
            }
        }
        ...
        <table cellpadding="0" cellspacing="0" class="form_table">
            ...
            <tr class="tr_dark">
                ...
                <td class="form_content">
                    ...
                    <input type="button" value="切换为普天身份证阅读器" class="button_normal_long" style="width: 140px;background-repeat: repeat-x"  onclick="changeDevice(${id})">&nbsp;
            ...
            <tr class="tr_light">
                ...
                <td class="form_content" rowspan="5" style="width: 180px; height: 140px">
                    <div id="ocxPhoto">
                        <object classid="clsid:4B3CB088-9A00-4D24-87AA-F65C58531039" id="SynCardOcx1"
                                codeBase="${ctx}/tools/SynCardOcx.CAB#version=1,0,0,1" width="102" height="126">
                        </object>
                    </div>
                    <div id="tPhoto">
