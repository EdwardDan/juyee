...
<script type="text/javascript">
    $(function () {
        var conf = {
            gridOpts: {
                url: "${ctx}/projectBidSignDept/gridDataCustom.do?projectBidSignId=${projectBidSign.id}",
                colNames: ['ID',
                    '购买标书单位',
                    '联系人',
                    '联系电话',
    ...
    function doView(id) {
        openWindow("查看招标购买标书单位", "${ctx}/projectBidSignDept/view.do?id=" + id, false);
    }
    function goBack(){
        loadAjaxData("mainContent","${ctx}/projectBidSign/grid.do");
    }
    <c:if test="${canEdit}">
    function doAdd() {
        openWindow("添加招标购买标书单位", "${ctx}/projectBidSignDept/add.do?projectBidSignId=${projectBidSign.id}", true);
    }
    function doEdit(id) {
        openWindow("修改招标购买标书单位", "${ctx}/projectBidSignDept/modify.do?id=" + id, true);
    }
    function doDelete(id) {
        doGridDelete("${ctx}/projectBidSignDept/delete.do?id=" + id);
    }
    </c:if>
</script>
