...
<script type="text/javascript">
    $(function () {
        var conf = {
            gridOpts: {
                url: "${ctx}/projectCompleteQuery/gridDataCustom.do?flag=${flag}",
                colNames: ['ID','监督任务书编号' ,'报建编号', '项目名称', '建设地点', '项目分类', '总投资(万元)', '总面积(㎡)', '报建日期'
                    , '建设单位'
                    , "竣工日期"
                    , "备注"
                    , '操作'],
                colModel: [
                    ...
                    {name: 'ejt.jd_task_code', width: 30, align: "center", searchtype: "string", sortable: true},
                    {name: 't.project_name', width: 60, align: "left", searchtype: "string", sortable: true},
                    {name: 't.BUILD_ADDRESS', width: 50, align: "left", searchtype: "string", sortable: true},
                    {name: 't.BUILD_PROPERTY', width: 30, align: "center", searchtype: "string", sortable: true, hidden: true},
                    ...
                    {name: 't.total_Area', width: 35, align: "right", searchtype: "integer", sortable: true, formatter: 'currency', formatoptions: {decimalSeparator: ".", thousandsSeparator: ",",
                    ...
                    {name: 't.BUILD_UNIT_NAME', width: 5, align: "left", searchtype: "string", hidden: true},
                    {name: 'p.complete_Date', width: 30, align: "center", searchtype: "date", sortable: true, formatter: 'date', formatoptions: {srcformat: 'Y-m-d', newformat: 'Y-m-d'}},
                    {name: 'p.description', width: 40, align: "left", searchtype: "string"}
                ],
                actModel: [
                ...
                gridComplete: function () {  //在此事件中循环为每一行添加修改和删除链接
                    ...
                    for (var i = 0; i < ids.length; i++) {
                        ...
                        var jdTaskCode = rowData["ejt.jd_task_code"];
    ...
    function doViewSelect(id, flowCode) {
        openWindow("选择工地", "${ctx}/projectQuery/viewSelect.do?bjbh=" + id + "&flowCode=" + flowCode, false, 500, 300);
    }
