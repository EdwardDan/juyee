...
<script type="text/javascript">
    $(function () {
        var conf = {
            gridOpts: {
                url: "${ctx}/projectBidSign/gridDataCustom.do",
                colNames: ['ID',
                    '报建编号',
                    '标段号',
                    '项目名称',
                ...
                colModel: [
                    ...
                    {name: "buildUnit", width: "80", align: "left", searchtype: "string", sortable: true},
    ...
    function doView(id) {
        openWindow("查看招标登记", "${ctx}/projectBidSign/view.do?id=" + id, false, 800, 500);
    }
    function doRegister(id) {
        loadAjaxData("mainContent", "${ctx}/projectBidSignDept/grid.do?projectBidSignId=" + id);
    }
    <c:if test="${canEdit}">
    function doAdd() {
        openWindow("添加招标登记", "${ctx}/projectBidSign/add.do", true);
    }
    function doEdit(id) {
        openWindow("修改招标登记", "${ctx}/projectBidSign/modify.do?id=" + id, true);
    }
    function doDelete(id) {
        doGridDelete("${ctx}/projectBidSign/delete.do?id=" + id);
    }
    </c:if>
