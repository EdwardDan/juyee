...
<script type="text/javascript">
    $(function () {
        var conf = {
            gridOpts:{
                url:"${ctx}/sgAuditOpinion/gridDataCustom.do",
                colNames:['ID',
            	'序号',            	          	
            	'初审意见',            	          	
            	'复核意见',            	          	
...
{name:"updateUser",width:"88",align:"center",searchtype:"string",sortable:true}
    ...
    function doView(id) {
        openWindow("查看施工审核意见", "${ctx}/sgAuditOpinion/view.do?id=" + id, false);
    }
    <c:if test="${canEdit}">
    function doAdd() {
        openWindow("添加施工审核意见", "${ctx}/sgAuditOpinion/add.do", true);
    }
    function doEdit(id) {
        openWindow("修改施工审核意见", "${ctx}/sgAuditOpinion/modify.do?id=" + id, true);
    }    
    function doDelete(id) {
        doGridDelete("${ctx}/sgAuditOpinion/delete.do?id=" + id);
    }
    </c:if>
