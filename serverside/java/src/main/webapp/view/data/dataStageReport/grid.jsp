...
{name:"updateUser",width:"133",align:"center",searchtype:"string",sortable:true}
    ...
    function doView(id) {
        openWindow("查看办证推进填报", "${ctx}/dataStageReport/view.do?id=" + id, false);
    }
    <c:if test="${canEdit}">
    function doAdd() {
        openWindow("添加办证推进填报", "${ctx}/dataStageReport/add.do", true);
    }
    function doEdit(id) {
        openWindow("修改办证推进填报", "${ctx}/dataStageReport/modify.do?id=" + id, true);
    }    
    function doDelete(id) {
        doGridDelete("${ctx}/dataStageReport/delete.do?id=" + id);
    }
    </c:if>
