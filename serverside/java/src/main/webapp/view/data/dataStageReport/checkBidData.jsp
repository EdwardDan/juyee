        ...
        <c:if test="${fn:length(stageFirst.projStages)>0}">
            <c:forEach items="${stageFirst.projStages}" var="stageSecond" varStatus="senond">
                <tr class="tr_dark">
                    ...
                    <c:forEach items="${steps}" var="step">
                        <c:if test="${step.isValid}">
                            <c:set var="dataKey" value="${bidId}_${step.id}_${stageSecond.id}_jh"/>
                            <%--计划--%>
                            <td>
                                <select class="form_select_long" style="width: 99%;"
                                        name="resultCodeJH_${bidId}_${step.id}_${stageSecond.id}"
                                        id="resultCodeJH_${bidId}_${step.id}_${stageSecond.id}"
                                        onchange="changeResultJH(this.value,'${bidId}','${step.id}','${stageSecond.id}')">
                            ...
                            <td>
                                <textarea class="input_text" style="width: 99%;cursor: pointer;height: 50px;"
                                          readonly="true"
                                          id="dealDateJH_${bidId}_${step.id}_${stageSecond.id}"
                                          name="dealDateJH_${bidId}_${step.id}_${stageSecond.id}"
                                          title="${dataMap[dataKey].dealDate}"
                                          onclick="changeResult(document.getElementById('resultCodeJH_${bidId}_${step.id}_${stageSecond.id}').value,'${bidId}','${step.id}','${stageSecond.id}')">${dataMap[dataKey].dealDate}</textarea>
                            </td>
                            <%--实际--%>
                            <c:set var="dataKey" value="${bidId}_${step.id}_${stageSecond.id}_sj"/>
                            <td>
                                <select class="form_select_long" style="width: 99%;"
                                        name="resultCodeSJ_${bidId}_${step.id}_${stageSecond.id}"
                                        id="resultCodeSJ_${bidId}_${step.id}_${stageSecond.id}"
                                        onchange="changeResultSJ(this.value,'${bidId}','${step.id}','${stageSecond.id}')">
                                    <option value="">请选择</option>
                                    <c:forEach items="${results}" var="result">
                                        <option value="${result.code}"
                                                <c:if test="${dataMap[dataKey].resultCode == result.code}">selected</c:if>>${result.name}</option>
                            ...
                            <td>
                                <textarea class="input_text" style="width: 99%;cursor: pointer;height: 50px;"
                                          readonly="true"
                                          id="dealDateSJ_${bidId}_${step.id}_${stageSecond.id}"
                                          name="dealDateSJ_${bidId}_${step.id}_${stageSecond.id}"
                                          title="${dataMap[dataKey].dealDate}"
                                          onclick="changeResultSJ(document.getElementById('resultCodeSJ_${bidId}_${step.id}_${stageSecond.id}').value,'${bidId}','${step.id}','${stageSecond.id}')">${dataMap[dataKey].dealDate}</textarea>
                            </td>
                            <td>附件1</td>
                        </c:if>
                    </c:forEach>
            ...
            <tr class="tr_dark">
                ...
                <c:forEach items="${steps}" var="step">
                    <c:if test="${step.isValid}">
                        ...
                        <td>
                            ...
                            <div id="xblJH_${bidId}_${step.id}_${stageFirst.id}"
                                ...
                                计划上报：<input type="text" id="planSbDateJH_${bidId}_${step.id}_${stageFirst.id}"
                                            name="planSbDateJH_${bidId}_${step.id}_${stageFirst.id}" class="input_date" value="${dataMap[dataKey].planSbDateJH}">
                                <input type="button"
                                ...
                                计划批复：<input type="text" id="planPfDateJH_${bidId}_${step.id}_${stageFirst.id}"
                                            name="planPfDateJH_${bidId}_${step.id}_${stageFirst.id}" class="input_date" value="${dataMap[dataKey].planPfDateJH}">
                                <input type="button"
                                       class="button_calendar"
                                       value=""
                        ...
                        <td>
                            ...
                            <div id="xblSJ_${bidId}_${step.id}_${stageFirst.id}"
                                ...
                                计划上报：<input type="text" id="planSbDateSJ_${bidId}_${step.id}_${stageFirst.id}"
                                            name="planSbDateSJ_${bidId}_${step.id}_${stageFirst.id}" class="input_date" value="${dataMap[dataKey].planSbDateSJ}">
                                <input type="button"
                                       class="button_calendar"
                                       value=""
                                ...
                                计划批复：<input type="text" id="planPfDateSJ_${bidId}_${step.id}_${stageFirst.id}"
                                            name="planPfDateSJ_${bidId}_${step.id}_${stageFirst.id}" class="input_date" value="${dataMap[dataKey].planPfDateSJ}">
                                <input type="button"
                                       class="button_calendar"
                                       value=""
                        ...
                        <td>${upButton[dataKey2]}${dataMap[dataKey].downButtonSJ}</td>
                    </c:if>
                </c:forEach>
            </tr>
        </c:if>
