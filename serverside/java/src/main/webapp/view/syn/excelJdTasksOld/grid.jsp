...
<script type="text/javascript">
    $(function () {
        var conf = {
            gridOpts: {
                url: "${ctx}/excelJdTasksOld/gridDataCustom.do",
                colNames: ['ID',
                    '流转编号',
                    '监督任务书编号',
                    '报建编号',
...
//                流转编号、监督任务书编号、报建编号、项目名称、项目地点、报监日
                colModel: [
                    {name: 'id', width: 10, align: "center", searchtype: "integer", hidden: true},
                    {name: "flowCode", width: "35", align: "center", searchtype: "string", sortable: true},
                    ...
                    {name: "projectName", width: "70", align: "left", searchtype: "string", sortable: true},
...
//                        opButton += '<input type="button" value="编辑" onclick="doEdit(' + id + ')" class="button_normal"/> ';
    ...
    function doView(id) {
        openWindow("查看Excel监督单位工程", "${ctx}/excelJdTasks/view.do?id=" + id, false,1000,500);
    }
    <c:if test="${canEdit}">
    function doAdd() {
    ...
    <%--function doEdit(id) {--%>
        <%--openWindow("修改Excel监督单位工程", "${ctx}/excelJdTasks/modify.do?id=" + id, true);--%>
    <%--}--%>
    function doDelete(id) {
        doGridDelete("${ctx}/excelJdTasksOld/delete.do?id=" + id);
    }
    </c:if>
