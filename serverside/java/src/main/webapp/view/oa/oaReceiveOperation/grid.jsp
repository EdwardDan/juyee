...
<script type="text/javascript">
    $(function () {
        var conf = {
            gridOpts:{
                url:"${ctx}/oaReceiveOperation/gridDataCustom.do",
                colNames:['ID',
            	'操作编码',            	          	
            	'操作名称',            	          	
            	'是否有效',            	          	
...
{name:"code",width:"114",align:"center",searchtype:"string",sortable:true},
{name:"name",width:"114",align:"center",searchtype:"string",sortable:true},
{name:"isValid",width:"114",align:"center",searchtype:"integer",sortable:true,formatter:booleanFormat},
...
{name:"updateUser",width:"114",align:"center",searchtype:"string",sortable:true}
    ...
    function doView(id) {
        openWindow("查看收文管理流转操作", "${ctx}/oaReceiveOperation/view.do?id=" + id, false);
    }
    <c:if test="${canEdit}">
    function doAdd() {
        openWindow("添加收文管理流转操作", "${ctx}/oaReceiveOperation/add.do", true);
    }
    function doEdit(id) {
        openWindow("修改收文管理流转操作", "${ctx}/oaReceiveOperation/modify.do?id=" + id, true);
    }    
    function doDelete(id) {
        doGridDelete("${ctx}/oaReceiveOperation/delete.do?id=" + id);
    }
    </c:if>
