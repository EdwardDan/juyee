...
<script type="text/javascript">
    $(function () {
        var conf = {
            gridOpts:{
                url:"${ctx}/oaTaskHistory/gridDataCustom.do",
                colNames:['ID',
                    '任务类型',
                    '任务标题',
                    '创建时间',
                ...
                colModel:[
                    ...
                    {name:"task.createUser.displayName", width:"40", align:"center", searchtype:"string", sortable:true},
                    {name:"opDatetime", width:"40", align:"center", searchtype:"datetime", sortable:true, formatter:'date', formatoptions:{srcformat:'Y-m-d H:i:s', newformat:'Y-m-d H:i:s'}},
                ...
                gridComplete:function () {  //在此事件中循环为每一行添加修改和删除链接
                    ...
                    for (var i = 0; i < ids.length; i++) {
                        ...
                        var opButton = '<input type="button" value="查看" onclick="viewTask(\'\',\'' + rowData["task.type.viewUrl"] + rowData["task.referId"] + '\',\'' + rowData["task.type.isWorkflow"] + '\',true,true)" class="button_normal"/> ';
//                        opButton += '<input type="button" value="删除" onclick="doDelete(' + id + ')" class="button_normal">';
    ...
    function doDelete(id) {
        doGridDelete("${ctx}/oaTaskHistory/delete.do?id=" + id);
    }
</script>
