...
<script type="text/javascript">
    $(function () {
        var conf = {
            gridOpts: {
                url: "${ctx}/netMonitor/gridDataCustom.do",
                colNames: ['ID',
                    '网上受理编号',
                    '报建编号',
                    '标段编号',
...
//                        opButton += '<input type="button" value="编辑" onclick="doEdit(' + id + ')" class="button_normal"/> ';
    ...
    function doView(id) {
        openWindow("查看项目报监", "${ctx}/netCommon/view.do?bizClass=${className}&bizId=" + id, false);
    }
    <c:if test="${canEdit}">
    function doAdd() {
        openWindow("添加项目报监", "${ctx}/netMonitor/add.do?matterCode=${matterCode}", true,500,250);
    }
    function doEdit(id) {
        openWindow("修改项目报监", "${ctx}/netMonitor/modify.do?id=" + id, true);
    }
    function doDelete(id) {
        doGridDelete("${ctx}/netMonitor/delete.do?id=" + id);
    }
    function doAudit(id, stID) {
        openWindow("受理项目报监", "${ctx}/netCommon/audit.do?matterCode=${matterCode}&bizClass=${className}&bizId=" + id + "&acceptStatusId=" + stID, true);
    }
    </c:if>
