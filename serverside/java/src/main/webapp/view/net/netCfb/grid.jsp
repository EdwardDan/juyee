...
<script type="text/javascript">
    $(function () {
        var conf = {
            gridOpts: {
                url: "${ctx}/netCfb/gridDataCustom.do?fbTypeCode=${fbTypeCode}&bidMethod=${bidMethod}",
                colNames: ['ID',
//                    '网上受理编号',
                    '报建编号',
                    '标段编号',
...
//                    {name: "netCode", width: "40", align: "center", searchtype: "string", sortable: true},
                    ...
                    {name: "netDate", width: "25", align: "center", searchtype: "string", sortable: true},
...
//                        opButton += '<input type="button" value="删除" onclick="doDelete(' + id + ')" class="button_normal"/>';
    ...
    function doView(id) {
        openWindow("查看项目承发包", "${ctx}/netCommon/view.do?bizClass=NetCfb&bizId=" + id, false);
    }
    <c:if test="${canEdit}">
    function doAdd() {
        openWindow("添加项目承发包", "${ctx}/netCfb/add.do?fbTypeCode=${fbTypeCode}&matterCode=${matterCode}&bidMethod=${bidMethod}", true, 500, 250);
    }
    function doEdit(id) {
        openWindow("修改项目承发包", "${ctx}/netCfb/modify.do?matterCode=${matterCode}&fbTypeCode=${fbTypeCode}&bidMethod=${bidMethod}&id=" + id, true, 500, 250);
    }
    ...
    function doDelete(id) {
        doGridDelete("${ctx}/netCfb/delete.do?id=" + id);
    }
    </c:if>
    //按是否标注过滤数据
