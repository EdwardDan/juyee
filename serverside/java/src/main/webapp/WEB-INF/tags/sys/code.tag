<%@ tag pageEncoding="UTF-8" %><%--
 ...
 2、在需要系统代码的地方使用如下代码：
    <sys:code code="sys_unit_character" name="character" id="character" type="select" isSaveName="true"/>
    code一般使用com.justonetech.ipromis.Constants.SYS_UNIT_CHARACTER方式
    sysCodeDetailId一般在修改表单时使用，用于选中代码明细
    isSaveName -- 默认为false指存储syscodedetail.id，如果为true则存储syscodedetail.name
--%><%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"
        ...
        %><%@ attribute name="isNeedId" type="java.lang.Boolean" description="是否需要id，不需要id情况下可以用于循环数据输出"
    ...
    if (StringUtils.isNotEmpty(type)) {
        if ("select".equals(type)) {
            ret.append("<select");
            if(isNeedId == null || (isNeedId != null && isNeedId)){
                if (StringUtils.isEmpty(id)) {
                    id = name;
                }
                ret.append(" id=\"" + id + "\"");
            }
            ret.append(" name=\"" + name + "\"" + " onchange=\"" + StringUtils.defaultString(onChange) + "\"");
            ...
            for (SysCodeDetail sysCodeDetail : sysCodeDetails) {
                if (sysCodeDetail.getIsValid()) {
                    if(isSaveName != null && isSaveName){
                        ret.append("<option value=\"" + sysCodeDetail.getName() + "\"");
                    }else{
