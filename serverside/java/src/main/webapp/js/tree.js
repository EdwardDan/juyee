...
var treeActions = {
    ...
    moveUp:function (treeNode, zTree, className, condition) {
        ...
        $.get(CONTEXT_NAME + "/commonPage/moveUp.do?id=" + treeNode.id + "&clazz=" + className + param, function () {
            treeActions.refreshParentNode(treeNode, zTree);
        });
    },
    moveDown:function (treeNode, zTree, className, condition) {
        if (!isVaildTreeId(treeNode)) return;
        className = className || clazz;
        var param = "";
        if (condition != null) {
        ...
        $.get(CONTEXT_NAME + "/commonPage/moveDown.do?id=" + treeNode.id + "&clazz=" + className + param, function () {
            treeActions.refreshParentNode(treeNode, zTree);
        });
    },
...
function isVaildTreeId(treeNode) {
    return treeNode != null && treeNode.id != null;
//    return treeNode && treeNode.id && $.isNumeric(treeNode.id);
 ...
 * @param setting
 */
function initTree(ele, setting, zNodes) {
    var zTree;
    setting = setting || {};
    //简单配置
                ...
                if (treeNode) {
                    ...
                    if (info) {
                        if ($.isArray(info)) {
                            ...
                            loadAjaxData("treeRight", processUrl(info, treeNode, zTree));
                        }
                    }
                }
            },
...
function processTreeAction(text, treeAction, confirm, treeNode, zTree) {
    ...
    return function () {
        if (treeAction && confirm === true) {
            $.messager.confirm('提示框', '您确定要进行' + text + '操作？', function (bool) {
                if (bool) {
                    if (text.indexOf("删除") > -1 && treeNode.isParent) {
                        $.messager.alert("系统提示", "该节点下已经存在子节点，不能删除！", "warning");
                    } else {
                        //主要是判断删除的情况
