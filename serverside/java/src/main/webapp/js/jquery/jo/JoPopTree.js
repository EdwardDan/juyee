        ...
        { name: '确定',
            callback: function () {
                ...
                if(selectNodes.length>0&&!pop.options.check){
                    ...
                    if(pop.options.targetType != null){
                        if(pop.options.targetType != selectNodes[0].type){
                            alert("所选节点类型不对，请重新选择");
                            return false;
                        }
                    }
...
//                    pop.setType(selectNodes[0].type);
                    ...
                    $.each(checkNodes, function () {
                        if (!this.childs) {
                            if(pop.options.targetType != null){
                                if(pop.options.targetType == this.type){
                    ...
                    var selectEle = $(selectorAdapt(pop.options.selectObj));
                    if (selectEle.length > 0) {
                        removeRepeatOption($(selectorAdapt(pop.options.selectObj)).get(0));
                    }
                }
