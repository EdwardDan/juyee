...
//                width:1024,
    ...
    var adjustGridSize = (function () {
        ...
        var treeContainer = container.find("#treeRight");
        return function () {
...
//        由于div宽度在resize时还没有调整所以无法直接获取到调整后的宽度，故采用延时处理
        window.setTimeout(adjustGridSize, 500);
    });
    /*================end====初始化grid==============*/
        ...
        if ($("#pager2 div .ui-pg-div").html() == null) {
            $("#listGrid").jqGrid('navGrid', '#pager2', {del:false, add:false, edit:false, search:false, refresh:false}).navButtonAdd('#pager2', {
                ...
                onClickButton:function () {    //执行操作
                    ...
                    var actualIndexs = getActualIndexs("listGrid");
                    var filename = $('#listGrid').jqGrid('getGridParam', 'caption');
                    openWindow("导出到Excel", CONTEXT_NAME + "/commonPage/exportGrid.do?" + encodeURI("cols=" + cols + "&showtitles=" + showtitles + "&filename=" + filename+"&actualIndexs="+actualIndexs));
                },
                position:"first"  //按钮位置
            });
        }
...
//添加radio选项
function gridRadioFormat(cellvalue, options, rowObject) {
    return "<input type='radio' name='radioId' value='"+cellvalue+"'>";
}
function gridCheckRadio() {
