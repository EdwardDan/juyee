...
function tableFileQueued(file) {
    try {
        // You might include code here that prevents the form from being submitted while the upload is in
        // progress.  Then you'll want to put code in the Queue Complete handler to "unblock" the form
        var progress = new TableProgress(file, this.settings.grid);
        progress.setStatus("Pending...");
        progress.toggleCancel(true, this);
...
function tableUploadSuccess(file, serverData) {
    try {
        ...
        if(returnValue["attachmentID"]){
            progress.setComplete(returnValue["attachmentID"],this.settings.delete_url,this.settings.download_url,createTableTr);
        }else{
        ...
        if(returnValue["documentID"]&&this.settings.upload_url.indexOf(this.settings.documentId)<0)  {
            this.settings.upload_url +="&"+this.settings.documentId+"="+returnValue["documentID"];
