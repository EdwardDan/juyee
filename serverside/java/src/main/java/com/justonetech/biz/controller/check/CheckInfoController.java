 ...
 */
@Controller
public class CheckInfoController extends BaseCRUDActionController<CheckInfo> {
    ...
    public String grid(Model model) {
        ...
        return "view/check/checkInfo/grid";
    }
...
//            String sql = "select distinct t1.flow_code,t1.gd_code,t1.gd_name,t3.build_unit_name,t2.sg_total_unit_name,t3.PROJECT_NAME,t2.bjbh \n" +
//                    "from check_device_info t1,share_gd_info t2,share_pro_tobuild t3 \n" +
//                    "where t1.gd_code=t2.gd_code and t2.bjbh=t3.bjbh ";
            String sql = "select distinct t1.jd_task_code,t1.gd_code,t1.gd_name,t3.belong_area,t3.js_unit_name,t2.sg_total_unit_name,t2.bjbh \n" +
                    "from check_device_info t1,share_gd_info t2,excel_jd_tasks t3 \n" +
                    "where t1.gd_code=t2.gd_code and t2.bjbh=t3.bjbh ";
            if (regPerson != null) {
                String currentBjbh = regPersonManager.getBjbh(regPerson);
//                logger.debug("currentBjbh = " + currentBjbh);
                if (!StringHelper.isEmpty(currentBjbh)) {
                    sql += " and t2.bjbh='" + currentBjbh + "'";
                } else {
                    sql += " and 1=2";
                }
            }
            sql += " order by t1.jd_task_code desc";
...
//            logger.debug("query = " + query);
            ...
            for (Map data : rowList) {
                ...
                map.put("t3.js_unit_name", data.get("JS_UNIT_NAME"));
                ...
                String hql = "select distinct t.manager from CheckInfo t where t.gdCode= '" + data.get("GD_CODE") + "'" +
