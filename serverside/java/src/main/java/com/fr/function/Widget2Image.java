...
public class Widget2Image extends AbstractFunction {
    public Object run(Object[] args) {
        ...
        data = (FArray) args[2];
        String selection = new String(data.toString().replaceAll(",", ""));
        // 第四个参数:可选参数，按钮组对应的显示值数组
        FArray textArray = new FArray();
        if (args.length == 6 && args[3] instanceof FArray) {
            textArray = (FArray) args[3];
        }
        Float fontSize = ((Integer)args[4]).floatValue();
        int breakNum = (Integer) args[5];
        return new WidgetPaint(type, num, selection, textArray,fontSize,breakNum);
    }
    ...
    public static class WidgetPaint extends AbstractPainter {
        ...
        public WidgetPaint(String type, int num, String selection,
                           FArray textArray,Float fontSize,int breakNum) {
            this.type = type;
            this.num = num;
        ...
        public void paint(Graphics g, int width, int height, int resolution,
            ...
            g2d.setFont(FRFont.getInstance().applySize(fontSize));
            g2d.setPaint(Color.BLACK);
            int x = 2;
            int y = (height-imgHeights[0]*(num/breakNum+(num%breakNum==0?0:1)) )/ 2;
            String select = selection;
            for (int i = 0; i < num; i++) {
                if (select == null || select.length() < i+1) {
                    continue;
                }
                ...
                g2d.drawString(text, (float) x, (float) (y + GraphHelper.getFontMetrics(DEFUALT_FONT.applySize(fontSize))
                        .getAscent()));
                x += GraphHelper.getFontMetrics(DEFUALT_FONT.applySize(fontSize)).stringWidth(text) + 2;
                if((i+1)%breakNum==0){
                    x=2;
                    y += ( GraphHelper.getFontMetrics(DEFUALT_FONT.applySize(fontSize))).getHeight();
//                    y += ( GraphHelper.getFontMetrics(DEFUALT_FONT.applySize(fontSize))).getHeight() + 2;
                }
            }
        }
    }
