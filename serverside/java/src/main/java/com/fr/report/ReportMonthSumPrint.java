 ...
 * Date: 14-9-5
 */
public class ReportMonthSumPrint extends AbstractTableData {
    // 列名数组，保存程序数据集所有列名
    ...
    // 构造函数，定义表结构，该表有7个数据列，列名为column#0，column#1，。。。。。。column#6
    public ReportMonthSumPrint() {
        // 定义tableName参数
    ...
    public void init() {
        ...
        ReportMonthManager reportMonthManager = (ReportMonthManager) SpringContextUtil.getBean("reportMonthManager");

        valueList = reportMonthManager.getPrintList(JspHelper.getLong(id));
    }
